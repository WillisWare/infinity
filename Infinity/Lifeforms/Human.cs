...
{
    public class Human : Matter
    {
        public Human()
            : base(string.Empty, "Human", "Default")
        {
        ...
        {
            Name = string.Empty;
            var region = Choose(new Dictionary<string, double> { { "Western", 1.0 }, { "Hispanic", 1.0 }, { "Asian", 1.0 }, { "European", 1.0 } });
        ...
        {
            switch (Properties["Age Range"])
            {
                case "Baby":
                    return $"{Random.Next(1, 23)} Months";
                case "Child":
                    return $"{Random.Next(2, 12)} Years";
                case "Teenager":
                    return $"{Random.Next(13, 19)} Years";
                case "Young Adult":
                    return $"{Random.Next(20, 35)} Years";
                case "Adult":
                    return $"{Random.Next(36, 64)} Years";
                default:
                    return $"{Random.Next(65, 90)} Years";
            }
        }

        public string GetThoughts()
        {
            var uncomposed = new List<string>();
            ...
            foreach (var thought in uncomposed.Distinct())
            {
                thoughts += $"{thought} # & & & ";
            }
